...
/** Inline SVG icons for known platforms; fallback to a link icon */
...
export function SocialIcons({ links, className = '' }: SocialIconsProps) {
  ...
  return (
    <div className={`flex flex-wrap items-center justify-center gap-4 ${className}`} aria-label="Social links">
      {links.map((link) => (
        <a
          key={link.id}
          href={link.href}
          ...
          className="flex h-11 w-11 items-center justify-center rounded-full border border-pirate-gold/40 bg-pirate-charcoal/80 text-gray-400 transition hover:border-pirate-gold hover:bg-pirate-gold/10 hover:text-pirate-gold"
          title={link.name}
          aria-label={link.name}
        >
          <PlatformIcon id={link.id} className="h-5 w-5" />
        </a>
      ))}
    </div>
  );
